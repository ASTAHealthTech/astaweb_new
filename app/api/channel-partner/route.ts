...
function validatePayload(body: unknown): { ok: true; data: PartnerPayload } | { ok: false; error: string } {
  ...
  if (!institution) return { ok: false, error: "Organization name is required." };
  if (institution.length > 150) return { ok: false, error: "Organization name is too long." };
  ...
  if (state.length > 100) return { ok: false, error: "State is too long." };

  if (!message) return { ok: false, error: "Message is required." };
  if (message.length > 5000) return { ok: false, error: "Message is too long (max 5000 chars)." };

  return { ok: true, data: { fullName, workEmail, institution, phone, countryCode, country, state, message } };
}

// ── Email composition ───────────────────────────────────────────────
function composeEmail(data: PartnerPayload, ip: string): { subject: string; html: string; text: string } {
  const subject = `Partner Inquiry · ${data.country} · ${data.institution}`;
  ...
  <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden;">
    ...
    <div style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); padding: 28px 32px;">
      ...
      <p style="margin: 6px 0 0; font-size: 12px; color: rgba(255,255,255,0.55); font-family: monospace; text-transform: uppercase; letter-spacing: 0.1em;">
        ASTA Health Tech · Partnerships
      </p>
    </div>

    <!-- Inquiry type badge -->
    <div style="padding: 20px 32px 0;">
      <span style="display: inline-block; padding: 5px 14px; font-size: 12px; font-weight: 600; color: #10b981; background: rgba(16,185,129,0.08); border: 1px solid rgba(16,185,129,0.20); border-radius: 20px;">
        Channel Partner
      </span>
    </div>
    ...
    <div style="padding: 20px 32px 28px;">
      <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
        ...
        <tr style="border-bottom: 1px solid #f1f5f9;">
          <td style="padding: 12px 0; color: #64748b; font-weight: 500; vertical-align: top;">Organization</td>
        ...
        <tr style="border-bottom: 1px solid #f1f5f9;">
          <td style="padding: 12px 0; color: #64748b; font-weight: 500; vertical-align: top;">Country</td>
          <td style="padding: 12px 0; color: #0f172a; font-weight: 600;">${escapeHtml(data.country)}</td>
        </tr>
        <tr style="border-bottom: 1px solid #f1f5f9;">
          <td style="padding: 12px 0; color: #64748b; font-weight: 500; vertical-align: top;">State / Province</td>
          <td style="padding: 12px 0; color: #0f172a; font-weight: 600;">${escapeHtml(data.state)}</td>
        </tr>
      </table>
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("[channel-partner-api]", err);
    return NextResponse.json(
